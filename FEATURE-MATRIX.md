...
| **Gemma-4 dense** (E4B-it, 31B) | ✅ **WORKS on transformers v5** | Blocked on v4 (`model_type=gemma4`). With transformers 5.9.0 (clone venv): **gemma-4-E4B-it** (instruction-tuned dense) feature 10/10, ~552 prefill, ~5.5 decode. **gemma-4-31B** loads+serves+generates fine too, but it's a **BASE model** (no chat template) → use `/v1/completions`, not the chat suite. torch/vllm survived v5 dep bumps; Qwen still correct on v5 (regression ✅). |
