...
| **Gemma-4** (non-hybrid arch) | ✅ **WORKS on transformers v5** | Blocked on v4 (`model_type=gemma4` unrecognized). With transformers **5.9.0** (isolated clone venv): gemma-4-E4B feature 10/10, ~552 t/s prefill, ~5.5 t/s decode. torch/vllm survived the v5 dep bumps (numpy 2.4, hf_hub 1.16). Qwen configs still load on v5. 26B-A4B/31B pending bench. |
